...
class JSONDataLoader:
    ...
    def create_documents(self, text_fields: List[str] = None, metadata_fields: List[str] = None, id_field: str = None) -> List[Document]:
        """Convert JSON data to LangChain Documents"""
        if not text_fields:
            raise ValueError("❌ text_fields is required to extract content from JSON.")
    ...
    def analyze_structure(self) -> Dict[str, Any]:
        """Analyze JSON structure to suggest real field names"""
        if self.data is None:
            self.load_json()
        ...
        if isinstance(self.data, list) and self.data and isinstance(self.data[0], dict):
            ...
            analysis["structure"] = {k: type(v).__name__ for k, v in sample_item.items()}

            for key, value in sample_item.items():
                if isinstance(value, str):
                    analysis["suggested_text_fields"].append(key)
                elif isinstance(value, (int, float, bool)):
                    analysis["suggested_metadata_fields"].append(key)

        elif isinstance(self.data, dict):
            analysis["structure"] = {k: type(v).__name__ for k, v in self.data.items()}
            for key, value in self.data.items():
                if isinstance(value, str):
                    analysis["suggested_text_fields"].append(key)
                elif isinstance(value, (int, float, bool)):
